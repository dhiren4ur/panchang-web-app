...
class ForecastCalculator {
    ...
    public function generate(array $panchang, int $year=0, int $month=0, int $day=0): array {
        ...
        // ── Karana calculation ────────────────────────────────────────────────
        // 7 repeating Karanas cycle through tithis 2-29 (2 per tithi).
        // Tithi 1 morning = Kimstughna (fixed), Tithi 1 afternoon onwards = repeating cycle.
        // Tithi 30 last = Naga, Chatushpada, Shakuni (fixed ending karanas).
        $repeating = ['Bava','Balava','Kaulava','Taitila','Gara','Vanija','Vishti'];
        $full_tithi = ($paksha === 'Krishna') ? $tithi_num + 15 : $tithi_num;
        if ($full_tithi === 1) {
            $karana_name = 'Kimstughna';
        } elseif ($full_tithi === 29) {
            $karana_name = 'Shakuni';
        } elseif ($full_tithi === 30) {
            $karana_name = 'Chatushpada'; // morning; Naga in afternoon
        } else {
            // Each tithi has 2 karanas; repeating cycle starts at tithi 2 morning
            $karana_idx  = (($full_tithi - 2) * 2) % 7;
            $karana_name = $repeating[$karana_idx];
        }
        $karana_info  = $this->karana_data[$karana_name] ?? $this->karana_data['Bava'];
        $karana_score = $karana_info['score'];

        $yoga_note_en = $yoga_note_gu = '';
